...
var Player = (function (_super) {
    ...
    Player.prototype.handleClick = function (clickX) {
        if (clickX < d.canvas.width / 2) {
            if (this._curLane > d.Lane.LANE_LEFT)
                this._curLane -= 1;
            this._clickedPosition = Defines_1.Click_Position.POS_LEFT;
        }
        else {
            if (this._curLane < d.Lane.LANE_RIGHT)
                this._curLane += 1;
            this._clickedPosition = Defines_1.Click_Position.POS_RIGHT;
        }
    };
    Player.prototype.animationMove = function () {
        ...
        if (this.getPositionX() != goal) {
            if (this._clickedPosition == Defines_1.Click_Position.POS_LEFT)
